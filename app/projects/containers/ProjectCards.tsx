...
export default function ProjectCards({ projects }: ProjectCardsProps) {
  ...
  const handleLinkClick = (e: React.MouseEvent, link: Link) => {
    e.stopPropagation();
    if (link.type === 'github' || link.type === 'live') {
      window.open(
        link.type === 'github' 
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {projects.map((project, index) => (
        ...
        >
            ...
            {project.links.map((link, i) => (
              <li key={i}>
                <div 
                  ...
                  className="flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow cursor-pointer"
                >
                  {link.icon === 'github'  ? (
                  ...
                  {/* Conditionally render the availability tag based on isAvailableGit for github links and isAvailableWeb for live links */}
                  {link.type === 'github' && link.isAvailableGit && (
                    <span className="inline-flex items-center justify-center px-2 py-0.5 ms-3 text-xs font-medium rounded bg-green-100 text-green-800">
                      Available
                    </span>
                  )}
                  {link.type === 'github' && !link.isAvailableGit && (
                    <span className="inline-flex items-center justify-center px-2 py-0.5 ms-3 text-xs font-medium rounded bg-red-100 text-red-800">
                      Unavailable
                    </span>
                  )}
                  {link.type === 'live' && link.isAvailableWeb && (
                    <span className="inline-flex items-center justify-center px-2 py-0.5 ms-3 text-xs font-medium rounded bg-green-100 text-green-800">
                      Available
                    </span>
                  )}
          ...
          <div>
            <div className="inline-flex items-center text-xs font-normal text-gray-500">
